...
public final class ValidationUtility {
	...
	public static void checkRange(final LocalDate dataDa,final LocalDate dataA) {
		...
		if(dataDa.isAfter(dataA)) {
			throw new ValidationException("Valorizzare correttamente l'ordine delle date.");
		}

		if(dataDa.isAfter(LocalDate.now()) || dataA.isAfter(LocalDate.now())) {
			throw new ValidationException("Le date non possono essere maggiori rispetto alla data odierna.");
		}

		Period period = Period.between(dataDa, dataA);
		if(period.getMonths()>6) {
			throw new ValidationException("L'intervallo di ricerca può essere al massimo di 6 mesi.");
		}
	}
